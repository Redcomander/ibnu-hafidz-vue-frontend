  ...
  try {
    ...
    const timeline = [
      ...
      ...cashInNotes.map((item) => ({
        ...
        detail: `${item.sumber || 'Sumber'} • Rp ${Number(item.jumlah || 0).toLocaleString('id-ID')}`,
        date: item.tanggal,
        icon: 'document-text',
      })),
      ...notes.map((item) => ({
    ...
    if (!progressAreas.value.length && tukangList.length) {
      progressAreas.value = [
        { name: 'Gedung Utama', progress: 0 },
        { name: 'Parkir & Access', progress: 0 },
        { name: 'Ruang Kelas', progress: 0 },
      ]
    }
  } catch (error) {
    console.error(error)
