...
import {
  createRevitalisasiSmpNotaMasuk,
  deleteRevitalisasiSmpNotaMasuk,
  fetchRevitalisasiSmpNotaMasuk,
  updateRevitalisasiSmpNotaMasuk,
...
async function loadData() {
  loading.value = true
  try {
    const response = await fetchRevitalisasiSmpNotaMasuk({ search: search.value })
...
async function saveItem() {
  ...
  try {
    ...
    if (editingId.value) {
      await updateRevitalisasiSmpNotaMasuk(editingId.value, payload)
    } else {
      await createRevitalisasiSmpNotaMasuk(payload)
    }
...
async function deleteItem(id) {
  try {
    await deleteRevitalisasiSmpNotaMasuk(id)
    await loadData()
  } catch (error) {
    console.error(error)
