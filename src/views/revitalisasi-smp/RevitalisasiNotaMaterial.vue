  ...
  <div class="space-y-5">
      ...
      <div v-for="item in filteredItems" :key="item.id" class="glass-card p-4 rounded-2xl">
        ...
        <div class="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm text-gray-600">
          ...
          <div class="rounded-xl bg-gray-50 p-2.5">
            ...
            <p class="mt-1 font-medium">{{ formatRupiah(item.total_nilai) }}</p>
          </div>
          <div class="rounded-xl bg-gray-50 p-2.5">
      ...
      <div class="w-full max-w-xl max-h-[90vh] overflow-y-auto overscroll-contain bg-white rounded-2xl shadow-2xl p-4 md:p-6 animate-fade-in">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input :value="formatRupiahDisplay(form.total_nilai)" @input="handleCurrencyInput('total_nilai', $event)" type="text" inputmode="numeric" class="input-field" placeholder="Rp 0" />
          </div>
          <div>
              ...
              <label class="upload-dropzone" :class="{ 'has-file': selectedFiles.length || selectedFileName }">
                ...
                <div class="upload-content">
                  ...
                  <div class="upload-copy">
                    <span class="upload-title">{{ selectedFiles.length ? `${selectedFiles.length} foto dipilih` : 'Pilih foto nota' }}</span>
    ...
    <div v-if="showDetailModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div class="w-full max-w-2xl rounded-2xl bg-white p-5 shadow-2xl">
        <div class="mb-4 flex items-center justify-between">
          <h3 class="text-xl font-extrabold text-gray-900">Detail Nota Material</h3>
        ...
        <div v-if="detailItem" class="space-y-4 text-sm text-gray-700">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div class="rounded-xl bg-gray-50 p-3 md:col-span-2"><p class="text-[10px] uppercase tracking-[0.2em] text-gray-400">Supplier</p><p class="mt-1 font-medium text-gray-900">{{ detailItem.supplier || '-' }}</p></div>
            <div class="rounded-xl bg-gray-50 p-3"><p class="text-[10px] uppercase tracking-[0.2em] text-gray-400">Total Nilai</p><p class="mt-1 font-medium text-gray-900">{{ formatRupiah(detailItem.total_nilai) }}</p></div>
...
import {
  createRevitalisasiSmpNotaMaterial,
  deleteRevitalisasiSmpNotaMaterial,
  fetchRevitalisasiSmpNotaMaterial,
  updateRevitalisasiSmpNotaMaterial,
...
async function loadData() {
  loading.value = true
  try {
    const response = await fetchRevitalisasiSmpNotaMaterial({ search: search.value })
...
async function saveItem() {
  ...
  try {
    ...
    if (editingId.value) {
      await updateRevitalisasiSmpNotaMaterial(editingId.value, payload)
    } else {
      await createRevitalisasiSmpNotaMaterial(payload)
    }
...
async function deleteItem(id) {
  const confirmed = await confirmDelete({
    title: 'Hapus nota material?',
    message: 'Catatan nota material ini akan dihapus dari sistem. Lanjutkan?',
    confirmText: 'Hapus',
  })
  ...
  try {
    await deleteRevitalisasiSmpNotaMaterial(id)
    await loadData()
  } catch (error) {
    console.error(error)
